...
/// Model output stored separately from `HydrologyObservation`. No direction
/// is inferred from undirected buffered river geometry.
...
impl HydrologyWaterModelPage {
    pub fn validate(&self, evidence: &[u8]) -> Result<(), EnvironmentError> {
        ...
        for index in 0..len {
            ...
            let original = HydrologyKind::try_from(evidence[index])?;
