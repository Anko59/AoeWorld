use crate::{
    ENVIRONMENT_PAGE_SAMPLES, EnvironmentError, FieldPyramid, PotentialBiomePage,
...
pub(crate) struct PreparedBiome {
    samples_per_axis: u16,
    pages: BTreeMap<(u16, u16), PotentialBiomePage>,
}

pub(crate) fn level_zero_biome_pages(
    field: &FieldPyramid,
    pages: Vec<PotentialBiomePage>,
) -> Result<BTreeMap<(u16, u16), PotentialBiomePage>, EnvironmentError> {
    let mut levels = (0..field.levels.len())
        .map(|_| Vec::new())
        .collect::<Vec<Vec<PotentialBiomePage>>>();
    for page in pages {
    ...
    for (level, (metadata, level_pages)) in field.levels.iter().zip(levels).enumerate() {
        ...
        if level_pages.len() != usize::from(count).pow(2)
            || ordered_biome_page_root(&level_pages)? != metadata.ordered_page_root
        {
            return Err(EnvironmentError::InvalidPyramid);
        }
        if level == 0 {
...
impl PreparedBiome {
    ...
    pub(crate) fn class_at(&self, tile: TileCoord, width_tiles: i32) -> Option<u8> {
        ...
        (local_x < usize::from(page.width) && local_y < usize::from(page.height))
            .then(|| page.potential_biome_class[local_y * usize::from(page.width) + local_x])
    }
}
