use crate::{GroundMaterial, MapChunkGenerator};
...
pub fn find_path(
    ...
    if !walkable(terrain, origin) || !walkable(terrain, destination) {
    ...
    while let Some(current) = open.pop_first() {
        ...
        expansions += 1;
        for (neighbor, step_cost) in neighbors(terrain, tile) {
...
fn neighbors(terrain: MapChunkGenerator, tile: TileCoord) -> Vec<(TileCoord, u32)> {
    ...
    for delta_y in -1..=1 {
        for delta_x in -1..=1 {
            ...
            if !walkable(terrain, next) || !crossable(terrain, tile, next) {
                continue;
            }
            if delta_x != 0
                && delta_y != 0
                && (!walkable(terrain, TileCoord::new(tile.x + delta_x, tile.y))
                    || !walkable(terrain, TileCoord::new(tile.x, tile.y + delta_y)))
            {
                continue;
            }
