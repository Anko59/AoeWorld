...
use aoe_map::{
    CHUNK_TILES, GAME_TILE_METERS, GENERATION_RECIPE_VERSION, LayerProvenance, MapPackage,
};
...
pub enum ScaleQualificationError {
    ...
    #[error(
        "source scale package `{package_hash}` is not a recipe-5 source-backed {tiles_per_side}-tile 1:1 package"
    )]
    UnsupportedPackage {
        tiles_per_side: u64,
        package_hash: String,
...
fn validate_scale_package(
    ...
    if package.validate().is_err()
        || package.generation_recipe_version != GENERATION_RECIPE_VERSION
